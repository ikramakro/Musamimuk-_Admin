...
import 'bloc/add_fonts_bloc.dart';
import 'models/add_fonts_model.dart';

class AddFontsScreen extends StatelessWidget {
  AddFontsScreen({super.key});
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  static Widget builder(BuildContext context) {
    return BlocProvider<AddFontsBloc>(
      create: (context) => AddFontsBloc(AddFontsState(
        addFontsModelObj: AddFontsModel(),
      ))
        ..add(AddFontsInitialEvent()),
      child: AddFontsScreen(),
    );
  }
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return CustomAppBar(
      ...
      title: BlocSelector<AddFontsBloc, AddFontsState, TextEditingController?>(
  ...
  Widget _buildShapeNameColumn(BuildContext context) {
    return Container(
      width: double.maxFinite,
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child: BlocSelector<AddFontsBloc, AddFontsState,
                  ...
                  child: SizedBox(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        BlocSelector<AddFontsBloc, AddFontsState,
                            AddFontsModel?>(
                          selector: (state) => state.addFontsModelObj,
                          builder: (context, addFontsModelObj) {
                            return CustomDropDown(
                              onChanged: (value) {},
                              items: addFontsModelObj?.dropdownItemList ?? [],
                            );
                          },
                        )
                      ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 12.v),
          BlocSelector<AddFontsBloc, AddFontsState, TextEditingController?>(
            selector: (state) => state.writetagsoneController,
            builder: (context, writetagsoneController) {
              return CustomTextFormField(
                width: 182.h,
                controller: writetagsoneController,
  ...
  onTapSave(BuildContext context) {
    context.read<AddFontsBloc>().add(
          CreateAddEvent(
            onCreateAddEventSuccess: () {
              _onAddLogoEventSuccess(context);
            },
            onCreateAddEventError: () {
              _onAddLogoEventError(context);
            },
          ),
        );
  }

  void _onAddLogoEventSuccess(BuildContext context) {
    // Fluttertoast.showToast(
    //     msg: context.read<AddFontsBloc>().postAddLogoResp.message.toString() ??
    //         '');
  }

  void _onAddLogoEventError(BuildContext context) {
    // Fluttertoast.showToast(
    //   msg: "Error",
    // );
  }
