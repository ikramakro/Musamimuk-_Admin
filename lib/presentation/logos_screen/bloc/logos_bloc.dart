...
part 'logos_event.dart';
part 'logos_state.dart';

/// A bloc that manages the state of a Shapes according to the event that is dispatched to it.
// ignore_for_file: must_be_immutable
class LogosBloc extends Bloc<LogosEvent, LogosState> {
  LogosBloc(LogosState initialState) : super(initialState) {
    on<LogosInitialEvent>(_onInitialize);
  ...
  /// Calls the https://api.pixlerpro.com/api/logo/list API and triggers a [FetchListEvent] event on the [LogosBloc] bloc.
  ///
  /// The [BuildContext] parameter represents current [BuildContext]
  _onInitialize(
    LogosInitialEvent event,
    Emitter<LogosState> emit,
  ) async {
    emit(state.copyWith(
